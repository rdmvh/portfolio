...
VT100.prototype.initializeElements = function(container) {
  ...
  /*
  alert("margin:" + marginTop + ' ' + marginLeft + ' ' + (((window.innerWidth ||
                                  document.documentElement.clientWidth ||
                                  document.body.clientWidth) - marginRight)));
  alert("XXX:" + x + ' ' + y + ' ' + (0 + x + this.container.offsetWidth));
  */
  this.isEmbedded = false; // ugly workaround, if not scaled to 100% (ST)
  if (!this.isEmbedded) {
...
VT100.prototype.resizer = function() {
  ...
  // ST if (!newCursor.clientHeight) { // remove to make IE9 work
  if (false) {
